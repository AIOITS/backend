...
export class StnkService {
  ...
  async create(createStnkDto: StnkCreateInput) {
    ...
    if ( await this._prismaService.stnk.count({ where: {nomor_stnk: data.nomor_stnk} }) )
      throw new HttpException('Stnk sudah terdaftar', 400)

    if ( !await this._prismaService.pkb.count({ where: {nomor_pkb: data.nomor_pkb} }) )
      throw new HttpException('Nomor PKB tersebut belum terdaftar', 400)

    if ( await this._prismaService.stnk.count({ where: {nomor_pkb: data.nomor_pkb} }) )
      throw new HttpException('Stnk dengan nomor PKB tersebut sudah terdaftar', 400)

    if ( await this._prismaService.stnk.count({ where: {nomor_polisi: data.nomor_polisi} }) )
      throw new HttpException('Nomor Polisi telah terdaftar', 400)
    const nik = (await user).nik
    const createdStnk =
      await this._prismaService.stnk.create({
        data: {
          ...data,
